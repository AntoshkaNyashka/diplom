...
def build_route_templates() -> List[RouteTemplate]:
    """Типы маршрутов для каждого города, включая короткие и длинные поездки."""
    ...
    return [
        ...
        RouteTemplate("Недельный тур", "расширенный", {"экскурсии", "природа", "гастрономия", "музеи", "прогулки"}, 7, 10, "средняя", "авто", 1.10),
        RouteTemplate("Большое путешествие", "длительный", {"экскурсии", "природа", "история", "фото", "национальная культура"}, 12, 14, "средняя", "поезд", 1.05),
    ]
...
def passes_hard_filters(route: TouristRoute, profile: UserProfile) -> bool:
    ...
    # Для длинных поездок бюджет по умолчанию часто оказывается занижен.
    # Поэтому в жестком фильтре оставляем больше вариантов, а точный штраф
    # за превышение бюджета учитывается уже в budget_score().
    allowed_budget_multiplier = 2.0 if profile.days >= 8 else 1.25
    if route.price > profile.max_budget * allowed_budget_multiplier:
        return False

    # Для длинных отпусков допустимое отклонение больше: маршрут может быть
    # базовой программой, к которой турист добавляет свободные дни.
    allowed_duration_gap = 5 if profile.days >= 8 else 3
    if abs(route.duration_days - profile.days) > allowed_duration_gap:
        return False
